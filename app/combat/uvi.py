...
class UnitVsImprovement:
    ...
    def _calculate_roll_modifiers(self) -> None:
        ...
        if "Defensive Tactics" in self.defender.completed_research:
    ...
    def _execute_combat(self) -> None:
        ...
        if attacker_roll >= self.attacking_region.unit.hit_value:
            attacker_hit = True
        if defender_roll >= self.defending_region.improvement.hit_value:
            defender_hit = True
        ...
        if attacker_hit and not defender_hit:
            ...
            # take damage
            self.defending_region.improvement.health -= self.attacking_region.unit.victory_damage + self.attacker_damage_modifier
            ...
            # take damage
            self.attacking_region.unit.health -= self.defending_region.improvement.victory_damage + self.defender_damage_modifier
        else:
            # set result
            self.result = CombatOutcome.DRAW
            # take damage
            self.defending_region.improvement.health -= self.attacking_region.unit.draw_damage + self.attacker_damage_modifier
            self.attacking_region.unit.health -= self.defending_region.improvement.draw_damage + self.defender_damage_modifier
    ...
    def resolve(self) -> None:
        ...
        """
        self.war.log.append(f"{self.attacker.name} {self.attacking_region.unit.name} {self.attacking_region.id} vs {self.defender.name} {self.defending_region.improvement.name} {self.defending_region.id}")
